...
namespace FactoryFramework
{
    public class Conveyor : LogisticComponent, IInput, IOutput
    {
        ...
        public IPath p; // ? maybe should be included in serialization data
        ...
        public void CalculateCapacity(int _capacity=-1)
        {
            ...
            capacity = (_capacity == -1) ? Mathf.FloorToInt(Length / settings.BELT_SPACING) : _capacity;
        ...
        public void SetItemsOnBelt(ItemOnBelt[] newItems, int capacity)
        {
            items.Clear();
            CalculateCapacity(capacity);


            foreach (ItemOnBelt i in newItems)
            {
                ItemOnBelt iob = new ItemOnBelt()
                {
                    item = i.item,
        ...
        public void SetInputSocket(ConveyorSocket sock, Building b)
        {
            data.inputSocket = (IOutput) b;
            data.inputSocketIndex = b.GetOutputIndexBySocket(sock); // This seems counter-intuitive, but conveyor input socket is a building output socket
        }
        public void SetInputSocket(ConveyorBridge sock, Conveyor c)
        {
            data.inputSocket = (IOutput)c;
            data.inputSocketIndex = 0;
        }

        public void SetOutputSocket(ConveyorSocket sock, Building b)
        {
            data.outputSocket = (IInput)b;
            data.outputSocketIndex = b.GetInputIndexBySocket(sock);
        }
        public void SetOutputSocket(ConveyorBridge sock, Conveyor c)
        {
            data.outputSocket = (IInput)c;
            data.outputSocketIndex = 0;
        }
        public ConveyorBridge GetBridge() { return bridge ?? GetComponentInChildren<ConveyorBridge>(); }
    ...
    public struct ConveyorData
    {
        ...
        public IOutput inputSocket;
        public int inputSocketIndex;
        public IInput outputSocket;
