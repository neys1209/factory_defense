...
namespace FactoryFramework
{
    public class Processor : Building, IInput, IOutput
    {
        ...
        private bool CanStartProduction()
        {
            ...
            foreach (Item item in data.recipe.OutputItems)
            {
            ...
            for (int i = 0; i < data.recipe.inputs.Length; i++)
            {
                Item item = data.recipe.InputItems[i];
                _inputs.TryGetValue(item, out int amount);
                if (amount < data.recipe.inputs[i].amount) return false;
            }
            return true;
        }
        private void ConsumeInputIngredients()
        {
            ...
            {
                Item item = data.recipe.InputItems[i];
                _inputs[item] -= data.recipe.inputs[i].amount;
            }
        }
        private void CreateOutputProducts()
        {
            for (int i = 0; i < data.recipe.outputs.Length; i++)
            {
                Item item = data.recipe.OutputItems[i];
                if (_outputs.ContainsKey(item))
                {
                    _outputs[item] = Mathf.Min(_outputs[item] + data.recipe.outputs[i].amount, item.itemData.maxStack);
                } else
                {
                    _outputs.Add(item, 1);
                }
